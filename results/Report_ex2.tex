...
by the method with higher satisfaction, which leads to a better result. Furthermore we sort methods by their satisfaction per time ratio, so
that the greedy algorithms do not need to iterate over all methods to find the best candidates.
...
We use a rank-based ACO variant.
...
% TODO some description of setups used: Rank-based/Max-Min, different -P values, with/wo local search (reference tables)
